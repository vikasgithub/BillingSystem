 ...
 * 
 * Created by IntelliJ IDEA. User: vikas Date: 20/10/12 Time: 7:35 PM To change
...
public class BillingServiceImpl implements BillingService {
	...
	public Bill generateBill(Order order) {
		if (order == null || order.isEmpty()) {
			throw new IllegalArgumentException(
					"Bill cannot be generated for an empty order.");
		}
	...
	private void applyPercentageBasedDiscount(Order order) {
		Discount percentageBasedDiscount = discountService
				.getUserBasedDiscount(order);
		List<Item> orderItems = order.getItems();
		for (Item item : orderItems) {
			if (!item.isGroceryItem()) { // percentage based discount are not
											// applicable to grocery items.
